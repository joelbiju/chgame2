...
function Play() {
  ...
  for (var i = 0; i < letterArr.length; i++) {
    ...
    inputElement.setAttribute('class', 'char');
    inputElement.setAttribute('type', 'text');
    inputElement.setAttribute('maxlength', '1');
    inputElement.addEventListener('input', handleInputChange);

    letterArr[i].innerHTML = "";
    letterArr[i].appendChild(inputElement);
  }
...
function handleInputChange(event) {
  ...
  if (nextInput) {
    ...
    // If there is no next input, check for puzzle completion
    var allInputs = document.querySelectorAll('.char');
    var puzzleCompleted = Array.from(allInputs).every(function (input) {
      return input.value.trim() !== '';
    });

    if (puzzleCompleted) {
      alert('Congratulations! Puzzle completed! Your code is: ' + generateUniqueCode());
...
function generateUniqueCode() {
  ...
  const codes = ['WR12Q5', 'WR137P', 'WR57I4', 'WR02NM'];

  // Randomly select a code from the array
